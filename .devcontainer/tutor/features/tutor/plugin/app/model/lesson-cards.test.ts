...
test("the lesson card lists every Rule by Feature, with live status, and links only the reached ones", () => {
  ...
  assert.equal(rules.length, fixtureLesson.homework.features.flatMap((feature) => feature.rules).length);
  ...
  const unlinked = lessonCardView({ ...fixtureLesson, coachThreadId: null });
...
test("a lesson ahead shows no focus", () => {
  const view = lessonCardView({ ...fixtureLesson, status: "ahead", progress: {} });
...
test("the Rule card draws the Rule's Examples from the live lesson", () => {
  const view = ruleCardView(fixtureLesson, focus, NOW);
  assert.ok(view !== null);
  ...
  assert.equal(ruleCardView(fixtureLesson, "no-such/rule", NOW), null);
});
