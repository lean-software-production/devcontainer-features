...
test("a lesson link can name a Rule, so it survives a new tab or a reload", () => {
  const path = coursePath({ kind: "start", homeworkId: "002" }, RULE);
  assert.equal(path, `start/002/${RULE}`);
  assert.deepEqual(parseCoursePath(path), { route: { kind: "start", homeworkId: "002" }, ruleKey: RULE });
  assert.deepEqual(
    parseCoursePath(`start/002/${encodeURIComponent(RULE)}`),
    { route: { kind: "start", homeworkId: "002" }, ruleKey: RULE },
    "an encoded slash reads the same",
  );
});

test("every other sub-path means what shared/routes.ts says, without a Rule", () => {
  assert.deepEqual(parseCoursePath("start/002"), { route: { kind: "start", homeworkId: "002" }, ruleKey: null });
  ...
  assert.equal(coursePath({ kind: "start", homeworkId: "002" }, null), "start/002");
});
