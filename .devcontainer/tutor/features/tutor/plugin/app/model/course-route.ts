...
// name a Rule to open ("lesson/002/<feature-slug>/<rule-slug>"), so Rule links
...
const LESSON_WITH_RULE = /^lesson\/(\d{3})\/(.+)$/;
...
export function parseCoursePath(subPath: string): CourseLocation {
  ...
  const match = LESSON_WITH_RULE.exec(trimmed);
  if (match !== null) {
    const ruleKey = decoded(match[2] ?? "");
    if (ruleKey !== null && RULE_KEY_PATTERN.test(ruleKey)) {
      return { route: { kind: "lesson", homeworkId: match[1] ?? "" }, ruleKey };
    }
  }
...
export function coursePath(route: TutorRoute, ruleKey: string | null = null): string {
  ...
  return route.kind === "lesson" && ruleKey !== null ? `${base}/${ruleKey}` : base;
}
